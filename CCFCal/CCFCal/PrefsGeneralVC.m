...
- (void)loadView
{
    ...
    MoVFLHelper *vfl = [[MoVFLHelper alloc] initWithSuperview:v metrics:@{@"m": @20} views:NSDictionaryOfVariableBindings(_login, firstDayLabel, _firstDayPopup, shortcutLabel, shortcutView, tvContainer, agendaDaysLabel, _agendaDaysPopup)];
    [vfl :@"V:|-m-[_login]-20-[_firstDayPopup]-20-[shortcutLabel]-3-[shortcutView(25)]-20-[tvContainer(170)]-[_agendaDaysPopup]-m-|"];
    [vfl :@"H:|-m-[_login]-(>=m)-|"];
...
- (void)viewWillAppear
{
    ...
    // Preserve Itsycal's escape hatch for users whose machines have trouble
    // querying login items. macOS 13+ uses SMAppService in MoLoginItem.
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DoNotCheckLoginItemStatus"] == NO) {
        _login.hidden = NO;
        _login.enabled = NO;
        [self refreshLoginItemStateAsync];
    }
    else {
        _login.hidden = YES;
    }
...
{
    NSError *error = nil;
    BOOL enable = login.state == NSControlStateValueOn;
    if (!MOSetLoginItemEnabled(enable, &error)) {
        login.state = MOIsLoginItemEnabled() ? NSControlStateValueOn : NSControlStateValueOff;
        NSAlert *alert = [NSAlert new];
        alert.messageText = NSLocalizedString(@"Unable to update the login item.", @"");
        alert.informativeText = error.localizedDescription ?: NSLocalizedString(@"Open System Settings and enable CCFCal under Login Items.", @"");
        [alert addButtonWithTitle:NSLocalizedString(@"Open Login Items", @"")];
        [alert addButtonWithTitle:NSLocalizedString(@"Cancel", @"")];
        if ([alert runModal] == NSAlertFirstButtonReturn) {
            MOOpenLoginItemsSettings();
        }
        return;
    }
    if (enable && MOLoginItemRequiresApproval()) {
        NSAlert *alert = [NSAlert new];
        alert.messageText = NSLocalizedString(@"Allow CCFCal in Login Items.", @"");
        alert.informativeText = NSLocalizedString(@"Open System Settings and allow CCFCal to run in the background so it can launch when you log in.", @"");
        [alert addButtonWithTitle:NSLocalizedString(@"Open Login Items", @"")];
...
- (void)refreshLoginItemStateAsync
{
    __weak typeof(self) weakSelf = self;
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
        BOOL enabled = MOIsLoginItemEnabled();
        dispatch_async(dispatch_get_main_queue(), ^{
            __strong typeof(weakSelf) strongSelf = weakSelf;
            if (!strongSelf || !strongSelf.isViewLoaded) {
                return;
            }
            strongSelf->_login.state = enabled ? NSControlStateValueOn : NSControlStateValueOff;
            strongSelf->_login.enabled = YES;
        });
    });
}
...
- (NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row
{
    ...
    if (!self.ec.calendarAccessGranted) {
        ...
        message.textField.stringValue = NSLocalizedString(@"Calendar access denied.\n\nCCFCal is more useful when it can display events from your calendars. You can change this setting in System Preferences › Security & Privacy › Privacy", @"");
        return message;
    }
