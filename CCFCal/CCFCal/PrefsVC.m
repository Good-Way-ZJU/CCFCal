...
static const CGFloat CCFCalPrefsMinimumContentWidth = 560.0;

static NSSize CCFCalPrefsContentSizeForView(NSView *view)
{
    NSSize size = view.fittingSize;
    size.width = MAX(size.width, CCFCalPrefsMinimumContentWidth);
    return size;
}
...
- (void)showAbout
{
    [self showTabWithIdentifier:NSLocalizedString(@"About", @"About prefs tab label")];
}

- (void)showPrefs
{
    NSString *aboutIdentifier = NSLocalizedString(@"About", @"About prefs tab label");
    if ([_toolbar.selectedItemIdentifier isEqualToString:aboutIdentifier]) {
        [self showTabWithIdentifier:NSLocalizedString(@"General", @"General prefs tab label")];
    }
}

- (void)showDDL
{
    [self showTabWithIdentifier:NSLocalizedString(@"DDL", @"DDL prefs tab label")];
}
...
- (void)setChildViewControllers:(NSArray<__kindof NSViewController *> *)childViewControllers
{
    ...
    [self.view setFrame:(NSRect){0, 0, CCFCalPrefsContentSizeForView(childViewControllers[0].view)}];
...
- (void)switchToTabForToolbarItem:(NSToolbarItem *)item animated:(BOOL)animated
{
    ...
    if (toVC) {
        ...
        NSRect contentRect = (NSRect){0, 0, CCFCalPrefsContentSizeForView(toVC.view)};
...
- (void)showTabWithIdentifier:(NSString *)identifier
{
    NSUInteger tag = [_toolbarIdentifiers indexOfObject:identifier];
    if (tag == NSNotFound) return;
...
- (nullable NSToolbarItem *)toolbar:(NSToolbar *)toolbar itemForItemIdentifier:(NSString *)itemIdentifier willBeInsertedIntoToolbar:(BOOL)flag
{
    ...
    if (item.image == nil && [itemIdentifier isEqualToString:NSLocalizedString(@"DDL", @"DDL prefs tab label")]) {
        item.image = [NSImage imageWithSystemSymbolName:@"calendar.badge.clock" accessibilityDescription:nil];
    }
    item.target = self;
