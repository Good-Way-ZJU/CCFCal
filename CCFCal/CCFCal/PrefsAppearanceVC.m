...
- (void)loadView
{
    ...
    _hideCountdown = chkbx(NSLocalizedString(@"Hide countdown", @""));
    _hideCountdown.target = self;
    _hideCountdown.action = @selector(hideCountdownChanged:);
    ...
    MoVFLHelper *vfl = [[MoVFLHelper alloc] initWithSuperview:v metrics:@{@"m": @20, @"mm": @40} views:NSDictionaryOfVariableBindings(iconPicker, menubarLabel, calendarLabel, separator0, separator1, showMonth, showDayOfWeek, showEventDots, useColoredDots, showWeeks, showLocation, showDaysWithoutEvents, _dateTimeFormat, helpButton, _hideIcon, _hideCountdown, highlight, themeLabel, themePopup, sizeMinLabel, sizeSlider, sizeMaxLabel)];
    [vfl :@"V:|-m-[menubarLabel]-10-[iconPicker]-[showMonth]-[showDayOfWeek]-[_dateTimeFormat]-[_hideIcon]-[_hideCountdown]-m-[calendarLabel]-10-[sizeSlider]-15-[themePopup]-m-[highlight]-m-[showEventDots]-[useColoredDots]-[showLocation]-[showDaysWithoutEvents]-[showWeeks]-m-|"];
    ...
    [_hideCountdown bind:@"value" toObject:[NSUserDefaultsController sharedUserDefaultsController] withKeyPath:[@"values." stringByAppendingString:kHideMenuBarCountdown] options:@{NSContinuouslyUpdatesValueBindingOption: @(YES)}];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(deadlineVisibilityChanged:) name:DDLSubscriptionsDidChangeNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(deadlineVisibilityChanged:) name:DDLCandidatesDidChangeNotification object:nil];
...
{
    NSString *dateTimeFormat = [_dateTimeFormat.stringValue stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    BOOL hasClockText = (dateTimeFormat.length > 0);
    BOOL hideCountdown = [[NSUserDefaults standardUserDefaults] boolForKey:kHideMenuBarCountdown];
    BOOL hasDeadlineCountdown = (!hideCountdown && [[DDLSubscriptionManager sharedManager] upcomingSubscribedDeadlinePayloadsWithLimit:1].count > 0);
    if (!hasClockText && !hasDeadlineCountdown) {
...
- (void)hideCountdownChanged:(id)sender
{
    [self updateHideIconState];
}

- (void)deadlineVisibilityChanged:(NSNotification *)notification
{
    [self updateHideIconState];
}
