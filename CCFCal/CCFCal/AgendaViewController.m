...
static BOOL AgendaInfoIsManagedDDL(EventInfo *info) {
    return info.isDDL && [info.event.notes containsString:@"managed_by:DDLCal"];
}

static NSString *AgendaDDLMetadataValue(NSString *key, EKEvent *event) {
    if (event.notes.length == 0 || key.length == 0) return @"";
    NSString *prefix = [key stringByAppendingString:@":"];
    for (NSString *line in [event.notes componentsSeparatedByCharactersInSet:[NSCharacterSet newlineCharacterSet]]) {
...
static NSString *AgendaDDLKindLabel(EKEvent *event) {
    NSString *kind = AgendaDDLMetadataValue(@"kind", event);
    if ([kind isEqualToString:@"journal"]) return NSLocalizedString(@"Journal", @"");
    if ([kind isEqualToString:@"conference"]) return NSLocalizedString(@"Conference", @"");
    return kind.capitalizedString ?: @"";
}

static NSString *AgendaDDLDomainsLabel(EKEvent *event) {
    NSString *domains = AgendaDDLMetadataValue(@"domains", event);
    return [domains stringByReplacingOccurrencesOfString:@"," withString:@" / "];
}

static NSString *AgendaDDLStageLabel(EKEvent *event) {
    NSString *stage = AgendaDDLMetadataValue(@"stage", event);
    if (stage.length == 0) return @"";
    NSString *normalized = stage.lowercaseString;
...
static NSString *AgendaCountdownString(NSDate *date) {
    if (!date) return @"";
...
static NSString *AgendaDDLSubtitle(EKEvent *event) {
    NSString *rank = AgendaDDLRankLabel(event);
    NSString *domains = AgendaDDLDomainsLabel(event);
    if (rank.length > 0 && domains.length > 0) {
        return [NSString stringWithFormat:@"%@ · %@", rank, domains];
    }
    return rank.length > 0 ? rank : domains;
}

@interface AgendaRowView : NSTableRowView
@property (nonatomic) BOOL isHovered;
@property (nonatomic) BOOL isDDL;
@property (nonatomic) NSColor *ddlAccentColor;
@end
...
@property (nonatomic) BOOL isDDLCard;
@property (nonatomic) NSColor *ddlAccentColor;
@end
...
- (void)viewDidLayout
{
    ...
    // in `CCFCalWindow` `-drawRect:`) is corrupted when:
...
- (NSTableRowView *)tableView:(MoTableView *)tableView rowViewForRow:(NSInteger)row
{
    ...
    id obj = self.events[row];
    rowView.isDDL = [obj isKindOfClass:[EventInfo class]] && AgendaInfoIsManagedDDL((EventInfo *)obj);
    rowView.ddlAccentColor = rowView.isDDL ? [[DDLSubscriptionManager sharedManager] highlightColorForEvent:((EventInfo *)obj).event] : nil;
    return rowView;
}
...
- (void)populateEventCell:(AgendaEventCell *)cell withInfo:(EventInfo *)info
{
    ...
    if (AgendaInfoIsManagedDDL(info)) {
        NSString *stage = AgendaDDLStageLabel(info.event);
        NSString *countdown = AgendaCountdownString(info.event.startDate);
        NSString *subtitle = AgendaDDLSubtitle(info.event);
        title = title.length > 0 ? title : NSLocalizedString(@"Paper Submission", @"");
        location = stage.length > 0 ? stage : @"";
        if (subtitle.length > 0 && location.length > 0) {
            location = [NSString stringWithFormat:@"%@ · %@", location, subtitle];
        } else if (subtitle.length > 0) {
            location = subtitle;
        }
        duration = countdown.length > 0 ? countdown : @"";

        [cell.grid rowAtIndex:1].hidden = (location.length == 0);
        [cell.grid rowAtIndex:2].hidden = NO;
        cell.btnVideo.enabled = NO;
        cell.btnVideo.hidden = YES;
        ...
        cell.ddlAccentColor = [[DDLSubscriptionManager sharedManager] highlightColorForEvent:info.event];
        cell.durationTextField.textColor = cell.ddlAccentColor ?: [NSColor systemRedColor];
        cell.dim = NO;
        cell.isDDLCard = YES;
        return;
    }
...
- (void)setDdlAccentColor:(NSColor *)ddlAccentColor
{
    _ddlAccentColor = ddlAccentColor;
    [self setNeedsDisplay:YES];
}
...
- (void)setIsDDLCard:(BOOL)isDDLCard
{
    if (_isDDLCard != isDDLCard) {
        _isDDLCard = isDDLCard;
        [self setNeedsDisplay:YES];
    }
}
...
- (void)drawRect:(NSRect)dirtyRect
{
    ...
    NSColor *dotColor = self.eventInfo.isDDL ? (self.ddlAccentColor ?: [NSColor systemRedColor]) : self.eventInfo.event.calendar.color;
...
- (void)populateWithEventInfo:(EventInfo *)info
{
    ...
    if (AgendaInfoIsManagedDDL(info)) {
        NSString *stage = AgendaDDLStageLabel(info.event);
        NSString *countdown = AgendaCountdownString(info.event.startDate);
        NSString *rank = AgendaDDLRankLabel(info.event);
        NSString *kind = AgendaDDLKindLabel(info.event);
        NSString *domains = AgendaDDLDomainsLabel(info.event);
        NSString *localTime = @"";
        if (info.event.startDate) {
            NSDateFormatter *formatter = [NSDateFormatter new];
            formatter.locale = [NSLocale currentLocale];
            formatter.timeZone = [NSTimeZone localTimeZone];
            formatter.dateStyle = NSDateFormatterMediumStyle;
            formatter.timeStyle = NSDateFormatterShortStyle;
            localTime = [formatter stringFromDate:info.event.startDate];
        }

        _title.stringValue = title;
        _duration.stringValue = localTime;
        _recurrence.stringValue = @"";
        NSMutableArray<NSString *> *detailLines = [NSMutableArray new];
        if (stage.length > 0) {
            [detailLines addObject:stage];
        }
        if (rank.length > 0 || kind.length > 0) {
            if (rank.length > 0 && kind.length > 0) {
                [detailLines addObject:[NSString stringWithFormat:@"%@ · %@", rank, kind]];
            } else {
                [detailLines addObject:(rank.length > 0 ? rank : kind)];
            }
        }
        if (domains.length > 0) {
            [detailLines addObject:domains];
        }
        if (countdown.length > 0) {
            [detailLines addObject:countdown];
        }
        NSString *details = [detailLines componentsJoinedByString:@"\n"];
        [self populateTextView:_location withString:details heightConstraint:_locHeight];

        [_grid rowAtIndex:1].hidden = NO;
        [_grid rowAtIndex:4].hidden = YES;
        [_grid rowAtIndex:5].hidden = YES;
        [_grid rowAtIndex:6].hidden = YES;
        [_grid rowAtIndex:7].hidden = YES;
        [_grid rowAtIndex:8].hidden = YES;
        [_grid rowAtIndex:9].hidden = YES;
