...
- (instancetype)initWithCalendar:(NSCalendar *)calendar delegate:(id<EventCenterDelegate>)delegate {
    ...
    if (self) {
        ...
        _queueWork = dispatch_queue_create("com.guwei.CCFCal.queueWork", DISPATCH_QUEUE_SERIAL);
        _queueIsol = dispatch_queue_create("com.guwei.CCFCal.queueIsol", DISPATCH_QUEUE_SERIAL);
        _queueIsol2 = dispatch_queue_create("com.guwei.CCFCal.queueIsol2", DISPATCH_QUEUE_SERIAL);
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:DDLSubscriptionsDidChangeNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *note) {
            __strong __typeof(weakSelf) strongSelf = weakSelf;
            if (!strongSelf) return;
            dispatch_async(strongSelf->_queueWork, ^{
                [strongSelf _filterEvents];
            });
        }];
        [[NSNotificationCenter defaultCenter] addObserverForName:DDLCalendarEventsDidSyncNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification *note) {
            __strong __typeof(weakSelf) strongSelf = weakSelf;
            if (!strongSelf) return;
            [strongSelf refetchAll];
        }];
    }
    return self;
}
...
- (BOOL)_eventIsDDL:(EKEvent *)event {
    if ([event.title containsString:kDDLTag]) return YES;
    if (event.notes && [event.notes containsString:kDDLTag]) return YES;
    return NO;
}

- (BOOL)_eventIsManagedDDL:(EKEvent *)event {
    return [event.calendar.title isEqualToString:DDLManagedCalendarTitle] || [event.notes containsString:@"managed_by:DDLCal"];
}
...
- (void)_filterEvents {
    ...
    for (NSDate *date in _eventsForDate) {
        for (EventInfo *info in _eventsForDate[date]) {
            if ([selectedCalendars containsObject:info.event.calendar.calendarIdentifier] || [self _eventIsManagedDDL:info.event]) {
                if (filteredEventsForDate[date] == nil) {
                    filteredEventsForDate[date] = [NSMutableArray new];
                }
                info.isDDL = [self _eventIsDDL:info.event] && [[DDLSubscriptionManager sharedManager] shouldHighlightEvent:info.event];
