...
- **DDL 卡片布局**：修复 DDL 卡片背景右侧留白不一致，以及窄宽度下阶段、CCF 等级和领域信息被过早省略的问题。
- **显示器切换后的 DDL 卡片刷新**：修复外接显示器、分辨率或屏幕布局变化后，议程中的 DDL 卡片可能没有及时重绘的问题。
...
- 网络访问用于自动刷新 DDL 数据。
- 1.0.2 不启用 Apple Events 和 App Groups 权限。
- 1.0.2 不启用 Sparkle 自动更新；App 本体更新需要手动下载新版 Release。
